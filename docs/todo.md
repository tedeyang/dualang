...
## P54: Smart Router 路由器（多 provider 统一调度）

从"主/兜底二元"升级到"N 条 provider 统一画像 + 评分 + 熔断"。源数据不再散落在 settings.apiKey / fallbackApiKey / PRESETS；改为 `chrome.storage.sync.providers[]`（每条含 baseUrl/model/enabled/label）+ `chrome.storage.local.perf:<id>` EWMA 画像 + `circuit:<id>` 状态机；apiKey 单独走 storage.local 读写（per-key IO lock 避免并发 RMW 丢失）。

**子任务（按 commit 时序）**：
- **P1 数据层**（5c6b169）：`router-types.ts` 定义 ProviderEntry / PerformanceProfile / CircuitRecord / CapabilityRecord 四类；`router/storage.ts` 一处统一 get/set + migration（老 settings.apiKey/baseUrl/model → 第一条 provider）；EWMA 合并函数在 recorder.ts
- **P2 Providers 管理 UI**（bc611fb）：popup "AI模型" tab 列 provider 卡片 + 添加 / 删除；hook 进 migration trigger（popup 打开时若发现 legacy 配置自动跑一次）
- **P3 Sampler 能力测试**（16fd66b）：每条 provider 发 5 条固定样本（单条 / 批量 / 流式），把 latency / throughput / englishLeak / jsonValid 等结果写入 `profile`；失败原因写进 `capability.batch = 'ok' | 'flaky' | 'broken'`
- **Sampler 阈值修复**（9f09994）：品牌密集文本（Anthropic / Claude / OpenAI / Vercel）合法译文也保留品牌名 → `englishLeak` 假阳性 → 大量 provider 被误判 broken。换成中性工程散文后问题消失
- **ApiKey backfill + per-key IO lock**（2b84634）：多 provider 共享 background 启动路径时 apiKey 写入竞态（后写覆盖前写）；加 `keyLocks: Map<id, Promise>` 串行化同一 key 的 RMW。同时补 smoke-test 脚本 `scripts/smoke-router.mjs`
- **P4 Recorder EWMA 画像**（9d996e8）：每次真实翻译都调 `recordSuccess(id, rttMs, tokens, quality)` → 滚动更新 profile；`tierOf(chars)` 把请求分 short/medium/long 三层分别记
- **P5 Service 抽象**（fda78ef）：`router/service.ts` 统一入口 `resolveSettings(base, ctx)` / `selectCandidates(ctx)`；handleTranslateBatch 入口一行替换，下游 api.ts 无感知。failover 模式等价旧主/兜底
- **Popup 翻译配置 tab + provider 编辑**（e6bd0cc）：routingMode 开关（failover / smart）+ 选主/次 provider 下拉；provider 卡片补 "编辑"按钮弹出表单
- **P6 Smart routing 评分 + 硬否决**（126ea39）：`scoring.ts` 计算 `score = wLatency × f(rtt) + wQuality × q + wCost × 1/cost + probeWeight`；`vetoSlot` 硬过滤（batch broken / context overflow / thinking 被用户关但模型强制思考）；preference 偏好轴（速度 / 质量 / 成本）调整权重
- **半开熔断**（349ad59）：CircuitRecord 状态 HEALTHY / COOLING / PROBING / PERMANENT_DISABLED；连续 failures 到阈值进 COOLING（冷却期屏蔽），冷却到期进 PROBING（`probeWeight < 1` 降分但允许少量探针流量），一次成功回 HEALTHY。`maybeUnfreeze` 懒惰解冻：读取时按当前时间判断是否跨门槛、转换后回写
- **P8 质量评分 V1**（543c18a）：`quality.ts` 算 `cjkRatio + englishLeak` 启发式 → recordSuccess 的 quality 字段；不需要金标集，现场 EWMA 滚动
- **P7 Health panel 在 status tab**（229a33a）：popup "状态" tab 展示每条 provider 的实时 score / rtt p95 / circuit state / last-error；路由器可观测性闭环

**设计约束**：
- 路由决策写 `router.select` 日志（mode / total / chosen / smart 模式 top3 分数）—— 排查"为什么走了这个 provider"必读
- `super-fine-chunk` 是专属 kind：不评分、primary → secondary → others 固定接力池（长文跨 chunk 需可预测的 provider 顺序避免限流交错）
- failover 路径下 router 未配好（空 providers / primary 无 key）→ `mergeSettingsWithCandidates` 返回 base 不变，等同"P5 以前"的旧 settings 路径

## P55: Popup / Bubble UI 大改（i18n + prompt 可编辑 + 统一 logger）

P54 落地后发现 popup 信息量爆炸（6 tabs，兜底+主从+流式+hedged 四套互斥）。本 PR 推倒重做 UI，统一配置模型为"AI模型列表 + routing mode"单一心智，并补齐国际化 + 日志观察。

**Popup 4-tab 新结构**（`popup.html` + `src/popup/index.ts`）：
- **翻译**：目标语言 + displayMode 分段（覆盖原文 / 整段追加 / 按行交替 × 高亮原文 / 高亮翻译）
- **AI模型**：provider 列表（label 选填）+ 卡片内联编辑 + HTML5 拖拽重排 + routing mode 选择（"默认首个模型，顺位替补" / "智能动态选择模型"）
- **状态**：模型健康面板（承接 P7 health）
- **高级**：UI 语言 + 自定义 prompt 编辑器 + 清除缓存 + 诊断区
- 删除：主（旧）tab、兜底&加速 tab、enableStreaming / maxTokens / hedged 相关配置；一次性 `chrome.storage.sync.remove` 清理这些 key

**Provider 卡片内联编辑**（`src/popup/providers-tab.ts`）：
- 老版本共享顶部表单，点任一卡片"编辑"都弹出同一个编辑框（bug）→ 改成 per-card 嵌入式 form，id 稳定，label/baseUrl/model/apiKey 全字段可改
- 拖拽 reorder 用 native HTML5 drag events，放下时 `saveProviders` 持久化并打 `providers.reorder`
- CRUD 事件全部打日志：`providers.add / delete / toggle / reorder / edit.save`

**自定义 prompt（`src/background/custom-prompts.ts` + `profiles.ts`）**：
- 把 system prompt 拆成 intro / 用户可编辑区（translationRules / strict / boost）/ 协议段（`<tN>` 标记 + `---DICT---`）
- 用户在 popup 只能改中段；协议段是硬编码不让用户碰，避免误删标签导致解析崩。存储里对老 key（single / batch / dict / annotateDict）做静默 drop（不 migrate，直接用新默认）
- `{{LANG}}` 占位符由 composeSystemPrompt 在 runtime 替换

**i18n 架构分层**（`src/shared/i18n.ts` + `src/shared/i18n-popup.ts` + `src/popup/i18n-apply.ts`）：
- 核心包（content + bubble + CTA，约 16 key × 5 语言）留在 `i18n.ts`
- popup 专用字典（约 80 key × 5 语言）放 `i18n-popup.ts`，**不**被 content.js import → 避免给内容脚本注入 24kB 死重
- popup DOM 标记 `data-i18n` / `data-i18n-title` / `data-i18n-placeholder` / `data-i18n-aria-label` 四类；`applyI18n(root)` 扫描并替换
- 默认跟随系统 UI 语言（`detectDefaultUiLang`），高级 tab 可手动覆盖；切换即时 re-apply 无需刷新
- 英文 bubble 按钮文案经过缩写压缩防溢出（`Original` / `Translated` / `Both`），再靠分组头消歧义

**浮球新增联动**（`src/content/super-fine-bubble.ts`）：
- bubble 里的模型选择自动改写 router：`onPickModel` 把选中 provider 提到 index 0 + routingMode 切 failover + 同步老 settings 字段（兼容旧路径）
- 失败路径记 `bubble.provider.promote.fail` / `bubble.routing.set.fail`，不中断 UI

**统一 logger（`src/shared/logger.ts`）**：
- `log.debug / info / warn / error` 四级，事件名用 dot-分隔语义（`translate.request.ok` / `router.select` / `config.change.displayMode`）
- 30+ 旧 `console.*` 调用全部迁移；background 监听 `chrome.storage.onChanged` 打 `config.change.<key>` —— 用户改任何配置都留痕
- 统一前缀 `[Dualang]` 便于 DevTools 过滤

**Sampler 测试语料修复**：
- 移除品牌密集散文（Anthropic / Claude / OpenAI / Vercel …），改成中性工程英文
- 根因是 englishLeak 阈值把合法译文里保留的品牌名误判为"模型没翻译"，导致 Moonshot 等诚实模型反而 broken。换语料后所有 provider 测试恢复真实通过率

**commit**：57cf799 `feat: popup/bubble UI overhaul — i18n, prompt editing, unified logger`（32 files: +2370 / -1075，4 新文件）

**后续方向（未做）**：
- [ ] 浮球 i18n 目前覆盖 UI 字符串但精翻按钮文案仍是 hardcoded "精翻此文"，需要补齐
